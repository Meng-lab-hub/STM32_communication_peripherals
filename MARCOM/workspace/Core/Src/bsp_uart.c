...
HAL_StatusTypeDef UART_Init()
{
	...
	return HAL_UART_Receive_IT(&huart3, (uint8_t*)&rxBuffer, 1);
}
...
HAL_StatusTypeDef UART_SendString(char *str)
{
	...
	while ((huart3.gState != HAL_UART_STATE_BUSY_RX) && (huart3.gState != HAL_UART_STATE_READY))
	{
	}
	...
	//strncpy(txBuffer, str, TXBUFFERSIZE); // --> Bug (?), it causes a HardFault sometimes...
	//uint32_t length = strlen(txBuffer); // --> Bug (?), it causes a HardFault sometimes...
	uint16_t length = 0;
	...
	return HAL_UART_Transmit_IT(&huart3, (uint8_t*)txBuffer, length);
}
