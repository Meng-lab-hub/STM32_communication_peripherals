 ...
 * call the generated initialization function here in the CAN_Init() function.
...
HAL_StatusTypeDef CAN_Init()
{
	...
	// - Instance: CAN1
	// - Prescaler: 4
	// - First bit segment is 6 quantums, second is 8 quantums
	...
	// Next the message filter has to be set. In our case this is enables the
	// reception of incoming messages on FIFO0. This can be done by filling a
	// struct variable of type CAN_FilterTypeDef and calling HAL_CAN_FonfigFilter()
	// Note that the address of the filter descriptor has to be passed!
	// Set FilterBank and SlaveStartFilterBank to 0 (no consts for these.
	// Set filter to be active (ENABLE)
	// Use Fifo 0 (CAN_FILTER_FIFO0)
	...
	// After succesful configuration, start the CAN periphery using function HAL_CAN_Start()
...
HAL_StatusTypeDef CAN_SendMessage(uint8_t zoneID, uint8_t newStatus)
{
	...
	// with the parameters of the outgoing message, copy the data contents to the global txBuffer variable.
